...
def fetch(matchClass):
    req = urllib.request.Request("https://www.botzone.org/api/568bbc204ff80bad5906c38e/DEFAULT/localai")
...
if __name__=='__main__':
    ...
    while True:
        ...
        for mid, m in matches.items():
            ...
            if mid not in cnt :
                cnt[mid]=1
                data[mid]=[m.current_request]
            else : 
                cnt[mid]+=1
                data[mid].append(m.current_request)
            with open('tmp.in','wt') as f:
                print(cnt[mid],file = f)
                for x in data[mid] : 
                    print(x,file = f)

            with os.popen('python3 tetris.py < tmp.in') as p : 
                x = p.read()
            m.current_response=x[:-1]
            data[mid].append(m.current_response)
